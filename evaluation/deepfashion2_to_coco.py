...
VAL_ANNOS_DIR = ROOT / 'val_annos'
VAL_IMAGES_DIR = ROOT / 'val'
OUTPUT_JSON_PATH = ROOT / 'deepfashion2.json'

if not VAL_ANNOS_DIR.exists():
    raise FileNotFoundError(f"Missing annotation directory: {VAL_ANNOS_DIR}")
if not VAL_IMAGES_DIR.exists():
    raise FileNotFoundError(f"Missing image directory: {VAL_IMAGES_DIR}")
...
sub_index = 0
json_files = sorted(VAL_ANNOS_DIR.glob('*.json'))
if len(json_files) == 0:
    raise ValueError(f"No annotation JSON files found in {VAL_ANNOS_DIR}")

for json_path in json_files:
    num = int(json_path.stem)
    image_name = VAL_IMAGES_DIR / f"{num:06d}.jpg"
...
with open(OUTPUT_JSON_PATH, 'w') as f:
    json.dump(dataset, f)
print(f"COCO-style JSON written to: {OUTPUT_JSON_PATH}")
